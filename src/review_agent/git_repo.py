...
def git_source_from_clone_url(clone_url: str, branch: str) -> GitSource | None:
    """Build GitSource from configured clone URL and branch (base branch for PRs)."""
    url = clone_url.strip()
    if not url:
        return None
    b = branch.strip()
    ...
    if not parsed:
        logger.warning("Could not parse owner/repo from REVIEW_GIT_CLONE_URL: %s", url)
        return None
    owner, repo = parsed
    return GitSource(
        owner=owner,
...
def fetch_and_checkout_pr_head(repo_path: Path, pr_number: int, depth: int) -> None:
    """After cloning the base branch, fetch PR head and checkout (detached)."""
    repo = Repo(repo_path)
    ref = f"pull/{pr_number}/head"
    logger.info("Fetching %s from origin (depth=%s)", ref, depth)
